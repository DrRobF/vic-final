...
export function selectActiveAssignment(assignments, enrollment) {
  ...
  return (assignments || []).find((assignment) => {
    ...
    return status === 'assigned' && (!supportLevel || normalizeSupportLevel(assignment.mode) === supportLevel)
  }) || null
}
...
export function buildVicContext({ mode, lesson, assignmentMode, supportLevel, interest, gradeLevel }) {
  ...
  if (resolvedMode === STUDENT_DIRECTED) {
    return [
      ...
      safeInterest ? `- Saved student interest (optional personalization only): ${safeInterest}` : '',
    ].filter(Boolean).join('\n')
  }
  ...
  return [
    ...
    safeInterest ? `- Saved student interest (personalization only): ${safeInterest}` : '',
  ].filter(Boolean).join('\n')
}
